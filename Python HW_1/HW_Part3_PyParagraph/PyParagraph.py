...
for sentence in sentence_split:
    ...
    words_per_sentence.append(len(sents))

    
# Calculate the avg word count (per sentence)
#YOUR CODE HERE
#avg_sents = sum(words_per_sentence) / int(len(words_per_sentence))
avg_sents = sum(words_per_sentence) / float(len(words_per_sentence))
# Generate Paragraph Analysis Output
#YOUR CODE HERE
output = (f"Approximate Word Count: {word_count}\nApproximate Sentence Count: {sentence_count},\
          \nAverage Word Count: {avg_sents}, \nAverage Letter Count: {avg_letter_count}")
